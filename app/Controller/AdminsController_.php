...
class AdminsController extends AppController {
            ...
            if ($this->Auth->login()) {
                if ($this->Auth->user('status') == 'active') {
                    ...
                    if (!empty($duty[0]['roaster_details']['shift_name_time'])) {
                        ...
                        $fix_time = $duty[0]['roaster_details']['shift_name_time'];
                        ...
                        if ($fix_time = 'Morning (07.30 - 12.00)') {
                            ...
                            //  pr($fix_time); exit;
                            $time = substr($fix_time, 9, 5); //fix time  
                            ...
                            pr('here');
                            exit;
                        } elseif ($fix_time = 'Afternoon (12.00 - 20.00)') {
                            //  $fix_time = $agent[0]['users']['last_duty_sift'];
                            $s_time = substr($fix_time, 19, -1); //fix time 
                        } elseif ($fix_time = 'Night(20.00 - 02.00') {
                            // $fix_time = $agent[0]['users']['last_duty_sift'];
    ...
    function requested_si() {
        $this->loadModel('User');
        $si = $this->User->query("SELECT * FROM users
                left join roles on roles.id = users.role_id
                 WHERE log_status = 'request' AND role_id = 1 AND status = 'active'");
//        pr($agent); exit;
        $request_list = $si;
    ...
    function requested_agent() {
        ...
        if ($fix_time = 'Morning (07.30 - 12.00)') {
            ...
            //$fix_time = $agent[0]['users']['last_duty_sift'];
            //  pr($fix_time); exit;
            $s_time = substr($fix_time, 17, -1); //fix time  
        } elseif ($fix_time = 'Afternoon (12.00 - 20.00)') {
            //  $fix_time = $agent[0]['users']['last_duty_sift'];
            $s_time = substr($fix_time, 19, -1); //fix time 
        } elseif ($fix_time = 'Night(20.00 - 02.00') {
        ...
        //Time take in variable for validation end 
//        pr($s_time); exit;
//        if (!empty($agent[0]['users']['last_duty_sift'])) {
//            $ff = $agent[0]['users']['last_duty_sift'];
//            $ss = substr($ff, 17, -1);
//            if ($s_time == $ss) {
//                $request_list = $agent;
//            } else {
...
//pr($fix_time); exit;
    ...
    function attend_all() {
        $this->loadModel('RoasterDetail');
        $duty = $this->RoasterDetail->query("SELECT * FROM roaster_details WHERE attend_status = 'approve' order by user_id");
        $this->set(compact('duty'));
    }
...
//        exit;
        ...
        //data find out without approve,request end
        $msg = ' <div class="alert alert-success">
                                <button type="button" class="close" data-dismiss="alert">×</button>
                                <strong>You have successfully logged out</strong> 
                            </div>';
