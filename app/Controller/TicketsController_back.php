...
//        exit;
    ...
    function in_progress($page = 1) {
        ...
        $tickets = $this->Track->query("SELECT * FROM tracks tr
                        ...
                        WHERE t.status != 'outbound' OR t.status = 'open' ORDER BY tr.created DESC " . " LIMIT " . $offset . "," . $this->per_page);
    ...
    function promise_pay($page = 1) {
        ...
        $tickets = $this->Track->query("SELECT * FROM tracks tr
                        ...
                        INNER JOIN issues i ON tr.issue_id = i.id
                        INNER join package_customers pc on tr.package_customer_id = pc.id
                        WHERE `issue_id` = 204 and `status` = 'open' ORDER BY tr.created DESC  " . " LIMIT " . $offset . "," . $this->per_page);
        $temp = $this->Ticket->query("SELECT COUNT(tickets.id) as total FROM `tickets` WHERE `issue_id` = 204 and `status` = 'open'");
        $total = $temp[0][0]['total'];
        $total_page = ceil($total / $this->per_page);
